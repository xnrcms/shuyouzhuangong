...
class AccountLog extends Base
{
    ...
    private $mainTable          = 'user_account_log';
    ...
    private function listData($parame)
    {
		...
		//数据分页步长定义
		$modelParame['limit']		= isset($parame['limit']) ? $parame['limit'] : 25;
    ...
    private function listDataForAdmin($parame)
    {
        ...
        //Rtype :`INNER`、`LEFT`、`RIGHT`、`FULL`，不区分大小写，默认为`INNER`。
        $RelationTab                = [];
        $RelationTab['user_center'] = array('Ralias'=>'ud','Ron'=>'ud.id=main.uid','Rtype'=>'LEFT','Rfield'=>array('username as nickname'));
        ...
        //检索条件 需要对应的模型里面定义查询条件 格式为formatWhere...
        $modelParame['whereFun']    = 'formatWhereDefaultAdmin';
